...
const getWinners = async (clients: Client[]) => {
  ...
  const winnersWithoutIds = winners.map(winner => ({ name: winner.name, wins: winner.wins }))
  clients.forEach((client) =>
    client.send(prepareMessage(MESSAGE_TYPES_MAP.UPDATE_WINNERS, winnersWithoutIds)),
  );
};
